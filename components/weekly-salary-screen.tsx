...
/** Formats an ISO datetime string (e.g. "2026-07-13T09:02:15+05:30") as "9:02 AM" */
...
export function WeeklySalaryScreen() {
  ...
  const fetchData = async (range: { start: string; end: string }) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/payrolls/getWeeklySalary?fromDate=${range.start}&toDate=${range.end}`,
  ...
  const handleRowClick = async (record: ApiWeeklySalaryRecord) => {
    ...
    try {
      ...
      const results = await Promise.all(
        days.map(async (day) => {
          const res = await fetch(
            `${API_BASE_URL}/api/payrolls/getDailySalary?date=${day}`,
            { headers: { Authorization: `Bearer ${auth!.token}` } }
          )
          if (!res.ok) return []
  ...
  const handleConfirmGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/payrolls/GenerateWeeklySalary?anyDateInWeek=${d}`,
        { method: "POST", headers: { Authorization: `Bearer ${auth.token}` } }
      )
      if (!res.ok) throw new Error(`API error: ${res.statusText}`)
